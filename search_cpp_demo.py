...
# recompiling
params = {'protein': 'homo_sapien', 'num_protein': 1, 'num_qubits':10, 'Avg_CX': 0, 'Avg_Runtime(s)': 0}
n = 10
CX_search_counts = []
Runtimes_search = []
Fidelities = []
name_exp = datetime.now().strftime("%m_%d_%H_%M_%S")
directory_path = f'Results/SEARCH_Exp:_{name_exp}'
output_search_path = f'Results/SEARCH_Exp:_{name_exp}/Outputs_search'
os.makedirs(output_search_path, exist_ok=True)

for i in range(params['num_protein']):
    
    if "search_cpp" in os.listdir():
        os.remove("search_cpp")
    ...
    CX_search_counts.append(CX_count)
    Runtimes_search.append(runtime)
...
avg_cx = sum(CX_search_counts) / len(CX_search_counts)
avg_runtimes = sum(Runtimes_search) / len(Runtimes_search)
avg_fidelities = sum(Fidelities) / len(Fidelities)
csv_file_path = f'{directory_path}/experiment_results.csv'
with open(csv_file_path, 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['CX_counts'] + CX_search_counts)
    writer.writerow(['Avg_CX', avg_cx])
    writer.writerow(['Runtimes'] + Runtimes_search)
